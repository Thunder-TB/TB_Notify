...
1. To display a notification you should call it like below:
   1. Using Client Side:
    ```lua
       exports['TB_Notify']:Alert("Title", "Message", Time, 'type')
    ```
   2. Using Server Side:
    ```lua 
       TriggerClientEvent('TB_Notify:Alert', source, "Title", "Message", Time, 'type')
    ```
...
```lua
   function ESX.ShowNotification(message, type, length)
      if GetResourceState("TB_Notify") ~= "missing" then
         exports['TB_Notify']:Alert("NOTIFICATION", message, length, type)
      else
         print("[^1ERROR^7] ^5TB_Notify^7 is Missing!")
      end
   end
```
## TODO
